...
class MultiNet(object):
    ...
    def buildCNN(self):
        ...
        with tf.variable_scope('digit3'):
            h = tf.layers.dense(self.num2,units=self.CLASSNUM)
            dense = tf.layers.dense(hidden10, units=self.CLASSNUM)
            self.num3 = tf.add(h,dense)

        with tf.variable_scope('digit4'):
            h = tf.layers.dense(self.num3,units=self.CLASSNUM)
            dense = tf.layers.dense(hidden10, units=self.CLASSNUM)
            self.num4 = tf.add(h,dense)

        with tf.variable_scope('digit5'):
            h = tf.layers.dense(self.num4,units=self.CLASSNUM)
            dense = tf.layers.dense(hidden10, units=self.CLASSNUM)
            self.num5 = tf.add(h,dense)

        with tf.variable_scope('digit6'):
            h = tf.layers.dense(self.num5,units=self.CLASSNUM)
            dense = tf.layers.dense(hidden10, units=self.CLASSNUM)
            self.num6 = tf.add(h,dense)
        with tf.variable_scope('digit7'):
            h = tf.layers.dense(self.num6,units=self.CLASSNUM)
            dense = tf.layers.dense(hidden10, units=self.CLASSNUM)
            self.num7 = tf.add(h,dense)   
        with tf.variable_scope('length'):
            h = tf.layers.dense(self.num7,units=self.num_length)
            dense = tf.layers.dense(hidden10, units=self.num_length)
            self.len = tf.add(h,dense)
